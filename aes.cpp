...
void gen_sub_keys(const vector<unsigned char>& main_key, uint8_t* sub_keys, int Nk, int Nr)
{
	uint8_t tmp[4];
	uint8_t i, j;
	uint8_t len = Nb*(Nr + 1);
	...
	for (i = 0; i < Nk; i++) {
		sub_keys[4 * i + 0] = main_key[4 * i + 0];
		sub_keys[4 * i + 1] = main_key[4 * i + 1];
		sub_keys[4 * i + 2] = main_key[4 * i + 2];
		sub_keys[4 * i + 3] = main_key[4 * i + 3];
	}

	for (i = Nk; i < len; i++) {
		tmp[0] = sub_keys[4 * (i - 1) + 0];
		tmp[1] = sub_keys[4 * (i - 1) + 1];
		tmp[2] = sub_keys[4 * (i - 1) + 2];
		tmp[3] = sub_keys[4 * (i - 1) + 3];
		...
		sub_keys[4 * i + 0] = sub_keys[4 * (i - Nk) + 0] ^ tmp[0];
		sub_keys[4 * i + 1] = sub_keys[4 * (i - Nk) + 1] ^ tmp[1];
		sub_keys[4 * i + 2] = sub_keys[4 * (i - Nk) + 2] ^ tmp[2];
		sub_keys[4 * i + 3] = sub_keys[4 * (i - Nk) + 3] ^ tmp[3];
	}
}

void process_message(vector<unsigned char>& in_data_piece, uint8_t* sub_keys, OptType opt_type, int Nr, vector<unsigned char>& out_data_piece)
{
	if (opt_type==OptType::kEncrypt)
	{
		...
		for (i = 0; i < 4; i++) {
			for (j = 0; j < Nb; j++) {
				state[Nb*i + j] = in_data_piece[i + 4 * j];
			}
		}

		add_round_key(state, sub_keys, 0);

		for (r = 1; r < Nr; r++) {
			sub_bytes(state);
			shift_rows(state);
			mix_columns(state);
			add_round_key(state, sub_keys, r);
		}

		sub_bytes(state);
		shift_rows(state);
		add_round_key(state, sub_keys, Nr);

		for (i = 0; i < 4; i++) {
			for (j = 0; j < Nb; j++) {
				out_data_piece[i + 4 * j] = state[Nb*i + j];
			}
		}
	}
...
void do_crypt(const vector<unsigned char>& in_data, OptType opt_type, uint8_t * sub_keys, int piece_size, int Nr, vector<unsigned char>& out_data)
{
	out_data.clear();
	...
	while (true)
	{
		...
		if (block_count == number_of_blocks)
		{
			if (opt_type == OptType::kEncrypt)
			{
				...
				process_message(in_data_piece, sub_keys, opt_type, Nr, out_data_piece);
...
void main_aes()
{
	...
	// gen main key
	// aes key length 128(16) 192(24) 256(32)
	vector<unsigned char> main_key(24, 0);
	gen_key(main_key);
	print_vect(main_key, "main_key");

	// gen sub_keys
	int Nk, Nr;
	switch (main_key.size())
	{
	default:
	case 16:
		Nk = 4;
	...
	uint8_t *sub_keys = (uint8_t *)malloc(Nb*(Nr+1)*4);
	gen_sub_keys(main_key, sub_keys, Nk, Nr);
	...
	do_crypt(plain_data, OptType::kEncrypt, sub_keys, 16, Nr, cipher_data);
	print_vect(cipher_data, "cipher_data");

	vector<unsigned char> plain_data_from_cipher;
	do_crypt(cipher_data, OptType::kDecrypt, sub_keys, 16, Nr, plain_data_from_cipher);
